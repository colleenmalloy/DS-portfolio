...
Data %>% 
  ...
  ggplot(aes(x = Grade)) +
  geom_bar(aes(y = (after_stat(count)) / sum(after_stat(count)) * 100), fill = "#D7D3CC", color = "black") +
...
poo= filter(Data,`Cohort Code`=='NF')
for (X in indiv.courses) {
  avg_grade[X] = mean(poo$numerical_grade[poo[,X] == "1"], na.rm = TRUE)
  dfw[X]= sum(poo$Grade %in% c("D+", "D", "F", "VF", "W")& poo[,X] == "1")/ sum(poo[,X] == "1")*100
}
...
for (X in indiv.courses[order(avg_grade)]) {
  ...
  p <- ggplot(temp, aes(x = Grade))+
    geom_bar(aes(y = (after_stat(count))/sum(after_stat(count))*100), fill = "#1A1AFF", color = "black")+
